...
public:
  ...
  bool contains_recursive(const std::string& str, int current, int left,
                          int mid, int right)
  {
    if (right < left)
    {
...
int main()
{
  ...
  auto         indexes   = suffix_array.get_ordered_indexes();
  ...
  for (int i = 1; i < n; i++)
  {
    ...
    max_score = std::max(max_score, count * lcp_table[i]);
  }
